...
export default async function whatsappUpdate(req, res) {
  //check token
  try {
  ...
  try {
    await query(
      `UPDATE whatsapp_numbers SET phone_number_id = ${req.body.phoneNumberId} WHERE user_id = ${userId[0].id};`
    );
  } catch (err) {
    res.status(500).json({ error: "update whatsapp number error" });
    return;
  }
